...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compute cross-entropy and perplexity for texts.")
    parser.add_argument("--model", default="klusai/tf3-50M-base", help="HF model id or path")
    parser.add_argument("--input", default=None, help="Single input text")
    parser.add_argument("--file", default=None, help="Path to a text file (one sample per line)")
    parser.add_argument("--batch-size", type=int, default=8, help="Batch size for evaluation")
    parser.add_argument("--max-length", type=int, default=2048, help="Max sequence length (tokens)")
...
def compute_ce_ppl(
    ...
    with torch.no_grad():
        for start in range(0, len(texts), batch_size):
            ...
            shift_mask = enc["attention_mask"][:, 1:].contiguous() if "attention_mask" in enc else torch.ones_like(shift_labels)

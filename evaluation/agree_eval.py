...
def build_nlp():
    ...
    return stanza.Pipeline("ro", processors="tokenize,pos,lemma,depparse", tokenize_pretokenized=False, verbose=False)


def agree_feats(feats1, feats2, keys, word1=None, word2=None):
    """Return True if all keys match in both feats dicts"""
    for k in keys:
        if feats1.get(k) != feats2.get(k) and feats1.get(k) != None and feats2.get(k) != None:
    ...
    for word in sentence.words:
        ...
        if word.deprel == "nsubj" and head.upos == "VERB":
            ...
            # participle verb: check auxiliary instead
                ...
                for aux in [w for w in sentence.words if w.head == head.id and w.deprel == "aux"]:
